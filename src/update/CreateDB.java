...
public class CreateDB 
{
	...
	static StringBuffer InsertQuery1 = new StringBuffer("insert into PathDB2(version, Source, Region, ASNum, ASName, IsSent");
	...
	{
		System.out.println("started main from NIXIUpdate");
		try {
			setUpDatabase();
	...
	static String GetISPName(String IP, String Region) throws ClassNotFoundException, SQLException, HttpException, IOException
	{
		...
		Connection conn = DriverManager.getConnection("jdbc:h2:file:"+userHome, "sa", "");
		System.out.println("path -  jdbc:h2:file:"+userHome);
        Statement stat = conn.createStatement();
	...
	static void GetRoutes(String IP, Boolean isSent, String ISPName, String Region) throws HttpException, IOException, ClassNotFoundException, SQLException
	{
		...
		String ASName = ISPName.substring(0,ISPName.indexOf(':'));
		String ASNum = ISPName.substring((ISPName.indexOf(':')+1),ISPName.length());
		InsertQuery4.append(Region+"','");
		InsertQuery4.append(ASNum+"','");
		InsertQuery4.append(ASName+"',");
	      ...
	      for (Element link : links) 
	      {
	    	  ...
	    	  catch(NumberFormatException e)
	    	  {
	    	      InsertQuery4.append("," + Integer.toString(hops)+",'"+Network+"'");
	    	      	...
	    	      	//System.out.println("query 4"+InsertQuery4);
	    	      	InsertQuery.append(InsertQuery4);
	    	      	InsertQuery.append(InsertQuery5);
	    	    
	    	      	if(hops>MaxHops)
	    	      	{	
	    	      		//System.out.println("the table has to be altered to accommodate hops more than 20");
	    	      		while(hops>MaxHops)
	    	      		{
	    	      			MaxHops++;
	    	      			stat.execute("alter Table PathDB2 add column Hop"+Integer.toString(MaxHops)+" varchar(255)");
	    	      			
	    	      			System.out.println("inserted column "+MaxHops+" into table");
	    	      		}
	    	      	}
